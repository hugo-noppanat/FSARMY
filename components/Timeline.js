...
export default function Timelines(props) {
  ...
  return (
    <VerticalTimeline lineColor={"var(--primary)"} layout={"1-column-left"}>
      {date.map((date, index) => {
        return (
          <VerticalTimelineElement
                  ...
                  <Fragment key={field.id}>
                    <Grid xs={12} key={field.id}>
                      <Input
                        ...
                        readOnly
                        {...register(
                          `timeline.${index}.date.${new Date(date)
                            .getDate()
                            .toString()}`
                        )}
                        css={{ fontSize: "25px" }}
                        initialValue={new Date(date).toLocaleDateString(
                          "th-TH",
                          {
                            year: "numeric",
                            month: "long",
                    ...
                    <Grid xs={12} key={field.id}>
                      <Row>
                        <DropdownInput
                          formName={`timeline.${index}.time.${new Date(date)
                            .getDate()
                            .toString()}`}
                          nameLabel={"ห้วงเวลา"}
                          menuItems={[
                        ...
                        <Input
                          ...
                          color="primary"
                          {...register(
                            `timeline.${index}.location.${new Date(date)
                              .getDate()
                              .toString()}`
                          )}
                        />
                    ...
                    <Grid xs={12} key={field.id}>
                      <Row>
                        <Textarea
                          ...
                          rows={5}
                          {...register(
                            `timeline.${index}.practice.${new Date(date)
                              .getDate()
                              .toString()}`
                          )}
                        />
                    ...
                    <Grid xs={12} key={field.id}>
                      <Row>
                        <Textarea
                          ...
                          rows={3}
                          {...register(
                            `timeline.${index}.note.${new Date(date)
                              .getDate()
                              .toString()}`
                          )}
                        />
